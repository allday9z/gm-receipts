...
async function submitToSheet(payload) {
  ...
  try {
    ...
    // Code.gs always returns explicit {ok: true/false, ...} JSON on both success
    // and failure — anything else (unparseable HTML, missing "ok", non-2xx) means
    // the deployed script isn't actually running our doPost (wrong/stale deploy,
    // Google auth/quota error page, etc). Require ok === true explicitly so a
    // broken deployment surfaces as a real error instead of a silent "success"
    // that drops the receipt on the floor.
